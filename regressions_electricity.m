...
X_pop = [POP];
b_pop_res = sar(y_res, X_pop, W);

X_dwell_1 = [DWELL];
b_dwell_1_res = sar(y_res, X_dwell_1, W);

X_dwell_2 = [DWELL OWN];
b_dwell_2_res = sar(y_res, X_dwell_2, W);

X_dwell_3 = [SF DU MF APT TH MH CV OTH];
b_dwell_3_res = sar(y_res, X_dwell_3, W);

X_pop_den = [DENS_RES];
b_pop_den_res = sar(y_res, X_pop_den, W);

b_age_1_res = sar(y_res, X_age_1, W);

b_age_2_res = sar(y_res, X_age_2, W);

b_age_3_res = sar(y_res, X_age_3, W);

b_room_res = sar(y_res, X_room, W);

b_eth_res = sar(y_res, X_eth, W);

b_inc_res = sar(y_res, X_inc, W);

% Run regressions for commercial electricity

X_emp = [EMP];
b_emp_com = sar(y_com, X_emp, W);

X_est_1 = [ESTAB];
b_est_1_com = sar(y_com, X_est_1, W);

X_est_3 = [AG FOR IND CON MAN WR COM INST ART MI GOV];
b_est_3_com = sar(y_com, X_est_3, W);

X_emp_den = [DENS_EMP];
b_emp_den_com = sar(y_com, X_emp_den, W);


% Run regressions for industrial electricity

b_emp_ind = sar(y_ind, X_emp, W);

b_est_1_ind = sar(y_ind, X_est_1, W);

b_est_3_ind = sar(y_ind, X_est_3, W);

b_emp_den_ind = sar(y_ind, X_emp_den, W);