...
% Run regressions for total transport

X_pop_emp = [POP EMP];
b_pop_emp_t = sar(y,X_pop_emp,W);

X_dwell_est_1 = [DWELL ESTAB];
b_dwell_est_1_t = sar(y,X_dwell_est_1,W);

X_dwell_est_2 = [DWELL ESTAB OWN];
b_dwell_est_2_t = sar(y,X_dwell_est_2,W);

X_dwell_est_3 = [SF DU MF APT TH MH CV OTH AG FOR IND CON MAN WR COM INST ART MI GOV];
b_dwell_est_3_t = sar(y,X_dwell_est_3,W);

X_pop_emp_den_1 = [DENS POP_DEN_R];
b_pop_emp_den_1_t = sar(y,X_pop_emp_den_1,W);

X_pop_emp_den_2 = [POP_DEN_R DENS_CAT_1 DENS_CAT_2];
b_pop_emp_den_2_t = sar(y,X_pop_emp_den_2,W);

X_age_1 = [AGE];
b_age_1_t = sar(y, X_age_1,W);

X_age_2 = [AGE_Old AGE_1961 AGE_1981 AGE_1991 AGE_2001 AGE_2006];
b_age_2_t = sar(y, X_age_2,W);

X_age_3 = [AGE_Old_2 AGE_1961_2 AGE_1981_2 AGE_1991_2 AGE_2001_2 AGE_2006_2];
b_age_3_t = sar(y, X_age_3, W);

X_room = [ROOMS_1 ROOMS_5 ROOMS_6 ROOMS_7 ROOMS_8];
b_room_t = sar(y, X_room, W);

X_eth = [AFR WE EE WA EA LA];
b_eth_t = sar(y, X_eth, W);

X_inc = [MED_INC];
b_inc_t = sar(y, X_inc, W);

X_mode = [HOV SOV ACT TRANS];
b_mode_t = sar(y, X_mode, W);

% Run regressions for per capita transport

b_pop_emp_tc = sar(y_cap,X_pop_emp,W);

b_dwell_est_1_tc = sar(y_cap,X_dwell_est_1,W);

b_dwell_est_2_tc = sar(y_cap,X_dwell_est_2,W);

b_dwell_est_3_tc = sar(y_cap,X_dwell_est_3,W);

b_pop_emp_den_1_tc = sar(y_cap,X_pop_emp_den_1,W);

b_pop_emp_den_2_tc = sar(y_cap,X_pop_emp_den_2,W);

b_age_1_tc = sar(y_cap, X_age_1,W);

b_age_2_tc = sar(y_cap, X_age_2,W);

b_age_3_tc = sar(y_cap, X_age_3, W);

b_room_tc = sar(y_cap, X_room, W);

b_eth_tc = sar(y_cap, X_eth, W);

b_inc_tc = sar(y_cap, X_inc, W);

b_mode_tc = sar(y_cap, X_mode, W);